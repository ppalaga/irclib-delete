...
public class IRCConnection extends Thread {
	private static class LoggingReader extends BufferedReader {
		 ...
		 * @param sniffer
		 */
		public LoggingReader(Reader in, TrafficLogger trafficLogger) {
			super(in);
	...
	private static class LoggingWriter extends PrintWriter {
		 ...
		 * @param sniffer
		 */
		public LoggingWriter(Writer out, TrafficLogger trafficLogger) {
			super(out);
			this.trafficLogger = trafficLogger;
		}
...
// ------------------------------
	...
	protected void prepare(Socket s) throws IOException {
		...
		if (trafficLogger != null) {
			in = new LoggingReader(new InputStreamReader(s.getInputStream(),
					encoding), trafficLogger);
			out = new LoggingWriter(new OutputStreamWriter(s.getOutputStream(),
					encoding), trafficLogger);
		} else {
